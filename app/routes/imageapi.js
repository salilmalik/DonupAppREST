		...
		function (req, res) {
			...
			if (validate === 'IMAGE VALIDATED') {
				...
				if (typeof watermarkText === 'undefined') {
					...
					console.log('watermarkText::' + watermarkText);
					gm(file.file.path).resize(200, 200).stroke("#ffffff").drawText(0, 0, watermarkText, "Center").autoOrient().write(
						imageThPath, function (err) {
							if (err) {
								console.log('error: ' + err);
							}
					...
					gm(file.file.path).fill("#ffffff").drawText(0, 0, watermarkText, "Center").autoOrient().write(
